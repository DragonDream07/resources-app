  ...
  async function fetchPromoCode() {
    ...
    try {
      ...
      setForm({
        ...
        expiry: data.expiry ? data.expiry.substring(0, 10) : '',
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.detail ?? data.message ?? 'Failed to update promo code');
      }
    ...
    return (
      <div className="admin-page">
        <div className="admin-page__error" role="alert">
          {loadError}
          <button className="btn btn--secondary" onClick={fetchPromoCode}>
            Retry
          </button>
        </div>
      </div>
