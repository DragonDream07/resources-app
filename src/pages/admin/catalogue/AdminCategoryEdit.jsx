...
export default function AdminCategoryEdit() {
  ...
  const loadCategories = () => {
    ...
    fetch('/categories').then(r => r.ok ? r.json() : Promise.reject()).then(d => { setCategories((d.categories || d || []).filter(c => c.id !== id)); setCategoriesLoading(false); }).catch(() => { setCategoriesError(true); setCategoriesLoading(false); });
  };
  ...
  useEffect(() => {
    ...
    fetch('/categories/' + id)
      ...
      .then(data => {
        setForm({ name: data.name || '', slug: data.slug || '', description: data.description || '', parentId: data.parentId || '' });
        setLoading(false);
      })
      .catch(() => { setSubmitError('Could not load category.'); setLoading(false); });
  }, [id]);
  ...
  const handleSubmit = (e) => {
    ...
    fetch('/categories/' + id, {
      ...
      .then(() => { setSuccessMsg('Category saved successfully.'); setSubmitting(false); })
      ...
      <div style={styles.container}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={styles.actions}>
            <button type="submit" style={styles.btn} disabled={submitting}>{submitting ? 'Saving...' : 'Save Category'}</button>
            <button type="button" style={styles.btnSecondary} onClick={() => navigate('/admin/catalogue/categories')}>Back to List</button>
          </div>
        </form>
      </div>
    </div>
